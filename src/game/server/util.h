...
class IEntityFactory;
	...
	#define SETUP_EXTERNC( mapClassName ) \
		extern "C" _declspec( dllexport ) IServerNetworkable* mapClassName();
#else
	#define SETUP_EXTERNC( mapClassName )
#endif
...
//			If player is not yet spawned or connected, returns nullptr
...
void UTIL_ScreenFadeAll( const Color& color, float fadeTime, float holdTime, int flags );
...
void UTIL_SayTextAll( const char* pText, CBasePlayer* pEntity = nullptr, bool bChat = false );
void UTIL_SayTextFilter( IRecipientFilter& filter, const char* pText, CBasePlayer* pEntity, bool bChat );
void UTIL_SayText2Filter( IRecipientFilter& filter, CBasePlayer* pEntity, bool bChat, const char* msg_name, const char* param1 = nullptr, const char* param2 = nullptr, const char* param3 = nullptr, const char* param4 = nullptr );
...
void UTIL_RestartAmbientSounds();
