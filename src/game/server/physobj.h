...
public:
	...
	virtual QAngle PreferredCarryAngles() { return m_angPreferredCarryAngles; }
...
public:
	...
	int GetNumAttachedObjects();
	float GetTotalMassAttachedObjects();
	CBaseEntity* GetAttachedObject( int iIndex );

	// Checking for hitting something
	void ResetHasHitSomething() { m_bHasHitSomething = false; }
	bool HasHitSomething() { return m_bHasHitSomething; }
