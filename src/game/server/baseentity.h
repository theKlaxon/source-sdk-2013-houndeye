...
public:
	...
	static inline bool Debug_IsPaused();
	static inline void Debug_SetSteps( int nSteps );
	static inline bool Debug_ShouldStep();
	static inline bool Debug_Step();
...
public:
	...
	virtual string_t GetModelName() const;
...
public:
	...
	bool IsCurrentlyTouching() const;
...
public:
	...
	inline edict_t* edict() { return NetworkProp()->edict(); }
	inline const edict_t* edict() const { return NetworkProp()->edict(); }
...
public:
	...
	void RemoveDeferred();// Sets the entity invisible, and makes it remove itself on the next frame
	...
	const char* GetDebugName();// do not make this virtual -- designed to handle NULL this
...
public:
	...
	virtual int RequiredEdictIndex() { return -1; }

	// interface function pts
	void ( CBaseEntity::*m_pfnMoveDone )();
...
public:
	...
	CAI_BaseNPC* MyNPCPointer();
	virtual CBaseCombatCharacter* MyCombatCharacterPointer() { return NULL; }
	virtual INextBot* MyNextBotPointer() { return NULL; }
	...
	virtual bool IsWearable() const { return false; }
	virtual CBaseCombatWeapon* MyCombatWeaponPointer() { return NULL; }

	// If this is a vehicle, returns the vehicle interface
	virtual IServerVehicle* GetServerVehicle() { return NULL; }

	// UNDONE: Make this data instead of procedural?
	virtual bool IsViewable();// is this something that would be looked at (model, sprite, etc.)?

	// Team Handling
	CTeam* GetTeam() const;                 // Get the Team this entity is on
	int GetTeamNumber() const;              // Get the Team number of the team this entity is on
	...
	bool IsInAnyTeam() const;               // Returns true if this entity is in any team
	const char* TeamID() const;             // Returns the name of the team this entity is on.
	...
	virtual CBaseEntity* GetEnemy() { return NULL; }
	virtual CBaseEntity* GetEnemy() const { return NULL; }
...
public:
	...
	virtual Vector GetSmoothedVelocity();
	...
	CBaseEntity* GetGroundEntity() const { return const_cast<CBaseEntity*>( this )->GetGroundEntity(); }
	...
	float GetShadowCastDistance() const;
...
private:
	...
	virtual Vector GetStepOrigin() const;
	virtual QAngle GetStepAngles() const;
...
public:
	...
	static CBasePlayer* GetPredictionPlayer();
...
//-----------------------------------------------------------------------------
inline bool CBaseEntity::Debug_IsPaused() {
...
//-----------------------------------------------------------------------------
inline bool CBaseEntity::Debug_Step() {
...
//-----------------------------------------------------------------------------
inline bool CBaseEntity::Debug_ShouldStep() {
...
//-----------------------------------------------------------------------------
inline CBaseEntity* CBaseEntity::GetMoveParent() {
	return m_hMoveParent.Get();
}

inline CBaseEntity* CBaseEntity::FirstMoveChild() {
	return m_hMoveChild.Get();
}

inline CBaseEntity* CBaseEntity::NextMovePeer() {
...
inline int CBaseEntity::GetSpawnFlags() const {
	return m_spawnflags;
}
...
inline void CBaseEntity::ClearSpawnFlags() {
	m_spawnflags = 0;
}
...
// checks to see if the entity is marked for deletion
//-----------------------------------------------------------------------------
inline bool CBaseEntity::IsMarkedForDeletion() {
...
//-----------------------------------------------------------------------------
inline const Vector& CBaseEntity::GetLocalOrigin() const {
	return m_vecOrigin.Get();
}

inline const QAngle& CBaseEntity::GetLocalAngles() const {
...
// Velocity
//-----------------------------------------------------------------------------
inline Vector CBaseEntity::GetSmoothedVelocity() {
	Vector vel;
...
inline float CBaseEntity::GetGravity() const {
	return m_flGravity;
}
...
inline float CBaseEntity::GetFriction() const {
	return m_flFriction;
}
...
inline float CBaseEntity::GetElasticity() const {
...
inline float CBaseEntity::GetShadowCastDistance() const {
	return m_flShadowCastDistance;
}

inline float CBaseEntity::GetLocalTime() const {
...
inline int CBaseEntity::GetTextureFrameIndex() {
...
inline void CBaseEntity::ClearSolidFlags() {
...
inline int CBaseEntity::GetSolidFlags() const {
...
inline string_t CBaseEntity::GetModelName() const {
	return m_ModelName;
}

inline int CBaseEntity::GetModelIndex() const {
...
//-----------------------------------------------------------------------------
inline Vector CBaseEntity::EyePosition() const {
	return const_cast<CBaseEntity*>( this )->EyePosition();
}

inline const QAngle& CBaseEntity::EyeAngles() const { // Direction of eyes in world space
	return const_cast<CBaseEntity*>( this )->EyeAngles();
}

inline const QAngle& CBaseEntity::LocalEyeAngles() const { // Direction of eyes
	return const_cast<CBaseEntity*>( this )->LocalEyeAngles();
}

inline Vector CBaseEntity::EarPosition() const// position of ears
{
