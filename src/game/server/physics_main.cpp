...
// Purpose:
//-----------------------------------------------------------------------------
CPhysicsPushedEntities::CPhysicsPushedEntities() : m_rgPusher( 8, 8 ), m_rgMoved( 32, 32 ) {
	m_flMoveTime = -1.0f;
}
...
public:
	...
	bool ShouldHitEntity( IHandleEntity* pHandleEntity, int contentsMask ) {
		auto* pTestEntity = assert_cast<CBaseEntity*>( pHandleEntity );
...
void CPhysicsPushedEntities::FinishRotPushedEntity( CBaseEntity* pPushedEntity, const RotatingPushMove_t& rotPushMove ) {
	...
	if ( pPushedEntity->IsPlayer() ) {
		...
		auto* player = dynamic_cast<CBasePlayer*>( pPushedEntity );
...
void CPhysicsPushedEntities::RestoreEntities() {
	// Reset all the pushed entities to get them back into place also
	for ( int i = m_rgMoved.Count(); --i >= 0; ) {
...
void CPhysicsPushedEntities::SetupAllInHierarchy( CBaseEntity* pParent ) {
	...
	CBaseEntity* pChild;
	for ( pChild = pParent->FirstMoveChild(); pChild != nullptr; pChild = pChild->NextMovePeer() ) {
...
void CPhysicsPushedEntities::RotateRootEntity( CBaseEntity* pRoot, float movetime, RotatingPushMove_t& rotation ) {
	...
	// rotate the pusher to its final position
...
CBaseEntity* CPhysicsPushedEntities::PerformRotatePush( CBaseEntity* pRoot, float movetime ) {
	...
	// and need to be pushed out of the way. Let's try to push them all out of the way.
...
void CBaseEntity::PhysicsDispatchThink( BASEPTR thinkFunc ) {
	...
	if ( thinkLimit ) {
		...
		if ( time > thinkLimit ) {
			// If it's an NPC print out the schedule/task that took so long
...
//			*steptrace - if not nullptr, the trace results of any vertical wall hit will be stored
		...
		if ( GetMoveType() == MOVETYPE_WALK && ( !( GetFlags() & FL_ONGROUND ) || GetFriction() != 1 ) ) { // reflect player velocity
			for ( i = 0; i < numplanes; i++ ) {
				if ( planes[ i ][ 2 ] > 0.7 ) {// floor or slope
					...
					PhysicsClipVelocity( original_velocity, planes[ i ], new_velocity, 1.f + sv_bounce.GetFloat() * ( 1 - GetFriction() ) );
				}
			}
...
//			interval.  Since that would affect a lot of code, and since most of
...
void CBaseEntity::PhysicsAddHalfGravity( float timestep ) {
	...
	Vector vecAbsVelocity = GetAbsVelocity();
	vecAbsVelocity[ 2 ] -= ( .5f * ent_gravity * GetCurrentGravity() * timestep );
...
//-----------------------------------------------------------------------------
// Purpose:  See if entity is inside another entity, if so, returns true if so, fills in *ppEntity if ppEntity is not nullptr
// Input  : **ppEntity - optional return pointer to entity we are inside of
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBaseEntity::PhysicsTestEntityPosition( CBaseEntity** ppEntity /*=nullptr*/ ) {
...
void CBaseEntity::PerformPush( float movetime ) {
	...
	if ( VPhysicsGetObject() ) {
		...
		if ( movetime > 0 && !m_pBlocker && GetSolid() == SOLID_VPHYSICS && g_pPushedEntities->CountMovedEntities() > 0 ) {
			...
			auto* pList = static_cast<physicspushlist_t*>( CreateDataObject( PHYSICSPUSHLIST ) );
			if ( pList ) {
...
//-----------------------------------------------------------------------------
void CBaseEntity::PhysicsPusher() {
...
//-----------------------------------------------------------------------------
void CBaseEntity::PhysicsNoclip() {
...
void CBaseEntity::PhysicsCustom() {
	...
	// Store off all the new state information...
...
void CBaseEntity::StepSimulationThink( float dt ) {
	...
	auto* step = static_cast<StepSimulationData*>( GetDataObject( STEPSIMULATION ) );
	if ( !step ) {
...
void CBaseEntity::PhysicsStep() {
	...
	float thinktime = static_cast<float>( thinktick ) * TICK_INTERVAL;
	...
	if ( HasDataObjectType( VPHYSICSUPDATEAI ) ) {
		auto* pUpdate = static_cast<vphysicsupdateai_t*>( GetDataObject( VPHYSICSUPDATEAI ) );
	...
	if ( VPhysicsGetObject() ) {
		if ( !VectorCompare( oldOrigin, GetAbsOrigin() ) ) {
			VPhysicsGetObject()->UpdateShadow( GetAbsOrigin(), vec3_angle, ( GetFlags() & FL_FLY ) != 0, dt );
		}
	}
...
// After this long, if a player isn't updating, then return its projectiles to server control
...
void Physics_RunThinkFunctions( bool simulating ) {
	...
	if ( !simulating ) {
		...
		auto** list = static_cast<CBaseEntity**>( stackalloc( sizeof( CBaseEntity* ) * listMax ) );
