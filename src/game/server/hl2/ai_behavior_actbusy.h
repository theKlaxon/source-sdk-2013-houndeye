...
public:
	...
	void Disable();
	void ForceActBusy( CAI_ActBusyGoal* pGoal, CAI_Hint* pHintNode = nullptr, float flMaxTime = NO_MAX_TIME, bool bVisibleOnly = false, bool bTeleportToBusy = false, bool bUseNearestBusy = false, CBaseEntity* pSeeEntity = nullptr, Activity activity = ACT_INVALID );
	void ForceActBusyLeave( bool bVisibleOnly = false );
	void StopBusying();
	bool IsStopBusying();
	CAI_Hint* FindActBusyHintNode();
	CAI_Hint* FindCombatActBusyHintNode();
	CAI_Hint* FindCombatActBusyTeleportHintNode();
	bool CanSelectSchedule();
	bool IsCurScheduleOverridable();
	...
	bool ActBusyNodeStillActive();
	bool IsMovingToBusy() { return m_bMovingToBusy; }
	bool IsEnabled() { return m_bEnabled; }
	float GetReasonableFacingDist() { return 0; }// Actbusy ignores reasonable facing
	bool IsInterruptable();
	bool ShouldPlayerAvoid();
	...
	bool IsInsideActBusy() { return m_bBusy; }
...
private:
	...
	int SelectScheduleForLeaving();
	int SelectScheduleWhileNotBusy( int iBase );
	int SelectScheduleWhileBusy();
