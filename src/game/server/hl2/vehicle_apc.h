...
public:
	...
	bool NPC_HasSecondaryWeapon() { return true; }
	...
	float Weapon_PrimaryCanFireAt();  // Return the time at which this vehicle's primary weapon can fire again
	float Weapon_SecondaryCanFireAt();// Return the time at which this vehicle's secondary weapon can fire again
};
...
public:
	...
	// Weaponry
	const Vector& GetPrimaryGunOrigin();
	void AimPrimaryWeapon( const Vector& vecForward );
	void AimSecondaryWeaponAt( CBaseEntity* pTarget );
	float PrimaryWeaponFireTime() { return m_flMachineGunTime; }
	float SecondaryWeaponFireTime() { return m_flRocketTime; }
