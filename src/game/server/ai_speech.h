...
{
public:
	virtual void OnSpokeConcept( AIConcept_t _concept, AI_Response *response )	{};
...
public:
	...
	bool Speak( AIConcept_t _concept, const char *modifiers = NULL, char *pszOutResponseChosen = NULL, size_t bufsize = 0, IRecipientFilter *filter = NULL );

	// These two methods allow looking up a response and dispatching it to be two different steps
	bool SpeakFindResponse( AI_Response &response, AIConcept_t _concept, const char *modifiers = NULL );
	bool SpeakDispatchResponse( AIConcept_t _concept, AI_Response &response, IRecipientFilter *filter = NULL );
	...
	bool CanSpeakConcept( AIConcept_t _concept );
	bool SpokeConcept( AIConcept_t _concept );
	float GetTimeSpokeConcept( AIConcept_t _concept ); // returns -1 if never
	void SetSpokeConcept( AIConcept_t _concept, AI_Response *response, bool bCallback = true );
	void ClearSpokeConcept( AIConcept_t _concept );
...
public:
	...
	virtual bool 	Speak( AIConcept_t _concept, const char *modifiers = NULL, char *pszOutResponseChosen = NULL, size_t bufsize = 0, IRecipientFilter *filter = NULL );

	// These two methods allow looking up a response and dispatching it to be two different steps
	bool			SpeakFindResponse( AI_Response& response, AIConcept_t _concept, const char *modifiers = NULL );
	bool 			SpeakDispatchResponse( AIConcept_t _concept, AI_Response& response );
	virtual void	PostSpeakDispatchResponse( AIConcept_t _concept, AI_Response& response ) { return; }
	float 			GetResponseDuration( AI_Response& response );
	...
	void SetSpokeConcept( AIConcept_t _concept, AI_Response *response, bool bCallback = true ) 		{ this->GetExpresser()->SetSpokeConcept( _concept, response, bCallback ); }
	float GetTimeSpokeConcept( AIConcept_t _concept )												{ return this->GetExpresser()->GetTimeSpokeConcept( _concept ); }
	bool SpokeConcept( AIConcept_t _concept )														{ return this->GetExpresser()->SpokeConcept( _concept ); }
...
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline bool CAI_ExpresserHost<BASE_NPC>::Speak( AIConcept_t _concept, const char *modifiers /*= NULL*/, char *pszOutResponseChosen /*=NULL*/, size_t bufsize /* = 0 */, IRecipientFilter *filter /* = NULL */ )
{
	AssertOnce( this->GetExpresser()->GetOuter() == this );
	return this->GetExpresser()->Speak( _concept, modifiers, pszOutResponseChosen, bufsize, filter );
}
...
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline bool CAI_ExpresserHost<BASE_NPC>::SpeakFindResponse( AI_Response& response, AIConcept_t _concept, const char *modifiers /*= NULL*/ )
{
	return this->GetExpresser()->SpeakFindResponse( response, _concept, modifiers );
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline bool CAI_ExpresserHost<BASE_NPC>::SpeakDispatchResponse( AIConcept_t _concept, AI_Response& response )
{
	if ( this->GetExpresser()->SpeakDispatchResponse( _concept, response ) )
	{
		PostSpeakDispatchResponse( _concept, response );
		return true;
	}
