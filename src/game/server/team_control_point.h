...
public:
	...
	virtual const char* GetControlPointMasterName() { return "team_control_point_master"; }
...
public:
	...
	int GetOwner() const;
	int GetDefaultOwner() const;
	bool RandomOwnerOnRestart() { return m_bRandomOwnerOnRestart; }

	void SetActive( bool active );
	inline bool IsActive() { return m_bActive; }
	void AnimThink();

	bool PointIsVisible() { return !( FBitSet( m_spawnflags, SF_CAP_POINT_HIDEFLAG ) ); }

	inline const char* GetName() { return STRING( m_iszPrintName ); }
	int GetCPGroup();
	int GetPointIndex() { return m_iPointIndex; }
	void SetPointIndex( int index ) { m_iPointIndex = index; }

	int GetWarnOnCap() { return m_iWarnOnCap; }
	string_t GetWarnSound() { return m_iszWarnSound; }

	int GetTeamIcon( int iTeam );

	int GetCurrentHudIconIndex();
	...
	bool HasBeenContested() const;// return true if this point has ever been contested, false if the enemy has never contested this point yet
	float LastContestedAt();
	void SetLastContestedAt( float flTime );

	void UpdateCapPercentage();
