...
public:
	...
	float GetUnlockTime() { return m_flUnlockTime; }
...
public:
	...
	void UpdateCapImage();
	bool IsPointLocked();
	int GetCapIndex() { return m_iCPIndex; }
	void SetSwipeUp( bool bUp ) { m_bSwipeUp = bUp; }
	bool ShouldSwipeUp() { return m_bSwipeUp; }
	int GetCapProgressDir() { return m_iCapProgressDir; }
	void SetCapProgressDir( int iDir ) { m_iCapProgressDir = iDir; }
	void FakePulse( float flTime );
	bool IsVisible();
	virtual void Paint();
	bool IsPointUnlockCountdownRunning();
