...
public:
	CViewRenderBeams();
	virtual ~CViewRenderBeams();

	// Implement IViewRenderBeams
public:
	virtual void InitBeams();
	virtual void ShutdownBeams();
	virtual void ClearBeams();

	// Updates the state of the temp ent beams
	virtual void UpdateTempEntBeams();

	virtual void DrawBeam( C_Beam* pbeam, ITraceFilter* pEntityBeamTraceFilter = nullptr );
...
const Vector& Beam_t::GetRenderOrigin() {
...
const QAngle& Beam_t::GetRenderAngles() {
	return vec3_angle;
}
...
bool Beam_t::ShouldDraw() {
	return true;
}

bool Beam_t::IsTransparent() {
	return true;
}
...
CViewRenderBeams::CViewRenderBeams() : m_pBeamTrails( 0 ) {
	m_pFreeBeams = nullptr;
	m_pActiveBeams = nullptr;
	m_nBeamFreeListLength = 0;
}

CViewRenderBeams::~CViewRenderBeams() {
	ClearBeams();
}
...
//-----------------------------------------------------------------------------
void CViewRenderBeams::InitBeams() {
...
//-----------------------------------------------------------------------------
void CViewRenderBeams::ClearBeams() {
	Beam_t* next = nullptr;
...
void CViewRenderBeams::ShutdownBeams() {
	if ( m_pBeamTrails ) {
...
void CViewRenderBeams::DrawBeam( C_Beam* pbeam, ITraceFilter* pEntityBeamTraceFilter ) {
	...
	#if defined( PORTAL )// Beams need to recursively draw through portals
