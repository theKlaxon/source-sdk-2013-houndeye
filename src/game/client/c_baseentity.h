...
public:
	...
	virtual ITraceFilter* GetBeamTraceFilter();
	virtual void DispatchTraceAttack( const CTakeDamageInfo& info, const Vector& vecDir, trace_t* ptr, CDmgAccumulator* pAccumulator = nullptr );
	virtual void TraceAttack( const CTakeDamageInfo& info, const Vector& vecDir, trace_t* ptr, CDmgAccumulator* pAccumulator = nullptr );
...
public:
	...
	virtual Vector GetObserverCamOrigin() { return GetRenderOrigin(); }// Return the origin for player observers tracking this target
	...
	virtual bool IgnoresZBuffer() const;
...
public:
	...
	C_BaseEntity* GetOwnerEntity() const;
	void SetOwnerEntity( C_BaseEntity* pOwner );

	C_BaseEntity* GetEffectEntity() const;
...
public:
	...
	inline IPhysicsObject* VPhysicsGetObject() const { return m_pPhysicsObject; }
...
public:
	...
	virtual const Vector& GetAbsOrigin() const;
	virtual const QAngle& GetAbsAngles() const;
	...
	virtual class CMouthInfo* GetMouth();
	...
	// Team handling
	virtual C_Team* GetTeam();
	virtual int GetTeamNumber() const;
	virtual void ChangeTeam( int iTeamNum );// Assign this entity to a team.
	virtual int GetRenderTeamNumber();
	virtual bool InSameTeam( C_BaseEntity* pEntity );// Returns true if the specified entity is on the same team as this one
	virtual bool InLocalTeam();

	// ID Target handling
	virtual bool IsValidIDTarget() { return false; }
	virtual const char* GetIDString() { return ""; };

	// See CSoundEmitterSystem
	virtual void ModifyEmitSoundParams( EmitSound_t& params );

	void EmitSound( const char* soundname, float soundtime = 0.0f, float* duration = nullptr );                            // Override for doing the general case of CPASAttenuationFilter( this ), and EmitSound( filter, entindex(), etc. );
	void EmitSound( const char* soundname, HSOUNDSCRIPTHANDLE& handle, float soundtime = 0.0f, float* duration = nullptr );// Override for doing the general case of CPASAttenuationFilter( this ), and EmitSound( filter, entindex(), etc. );
	...
	static void EmitSound( IRecipientFilter& filter, int iEntIndex, const char* soundname, const Vector* pOrigin = nullptr, float soundtime = 0.0f, float* duration = nullptr );
	static void EmitSound( IRecipientFilter& filter, int iEntIndex, const char* soundname, HSOUNDSCRIPTHANDLE& handle, const Vector* pOrigin = nullptr, float soundtime = 0.0f, float* duration = nullptr );
...
public:
	...
	C_BaseEntity* GetMoveParent() const;
	C_BaseEntity* GetRootMoveParent();
	C_BaseEntity* FirstMoveChild() const;
	C_BaseEntity* NextMovePeer() const;

	inline ClientEntityHandle_t GetClientHandle() const { return ClientEntityHandle_t( m_RefEHandle ); }
	inline bool IsServerEntity();
	...
	virtual bool IsVisibleToTargetID() const { return false; }
	virtual bool IsHealthBarVisible() const { return false; }
	...
	virtual bool IsClientCreated() const;
	...
	bool IsIntermediateDataAllocated() const;

	void InitPredictable();
	void ShutdownPredictable();

	virtual void SetPredictable( bool state );
	bool GetPredictable() const;
	...
	bool GetPredictionEligible() const;
...
public:
	...
	static const trace_t& GetTouchTrace();
...
public:
	...
	virtual bool IsBaseCombatCharacter() { return false; };
	virtual C_BaseCombatCharacter* MyCombatCharacterPointer() { return nullptr; }
	virtual bool IsNPC() { return false; }
	C_AI_BaseNPC* MyNPCPointer();
	virtual bool IsNextBot() { return false; }
	// TF2 specific
	virtual bool IsBaseObject() const { return false; }
	virtual bool IsBaseCombatWeapon() const { return false; }
	virtual class C_BaseCombatWeapon* MyCombatWeaponPointer() { return nullptr; }
	virtual bool IsCombatItem() const { return false; }

	virtual bool IsBaseTrain() const { return false; }
	...
	Vector EyePosition() const;          // position of eyes
	const QAngle& EyeAngles() const;     // Direction of eyes in world space
	const QAngle& LocalEyeAngles() const;// Direction of eyes
	Vector EarPosition() const;          // position of ears
	...
	//  this function, tests for nullptr, and Asserts m_pAttributes == dynamic_cast.
...
public:
	...
	static char const* GetDLLType();
	static void SetAbsQueriesValid( bool bValid );
	static bool IsAbsQueriesValid();
	...
	static bool IsAbsRecomputationsEnabled();
	...
	// Accessors for color.
	const Color GetRenderColor() const;
...
public:
	...
	bool IsDormantPredictable() const;
	bool BecameDormantThisPacket() const;
	...
	virtual C_BaseEntity* GetShadowUseOtherEntity() const;
	...
	bool IsCombatCharacter() { return MyCombatCharacterPointer() == nullptr ? false : true; }
...
protected:
	...
	virtual int GetStudioBody() { return 0; }
...
public:
	...
	float* GetRenderClipPlane();// Rendering clip plane, should be 4 floats, return value of nullptr indicates a disabled render clip plane
...
protected:
	...
	public:
		...
		bool EntityDeemedInvalid() { return ( m_bValidatedOwner && m_bDeemedInvalid ); }
...
//-----------------------------------------------------------------------------
inline bool C_BaseEntity::IsServerEntity() {
	return index != -1;
}
...
inline const model_t* C_BaseEntity::GetModel() const {
	return model;
}

inline int C_BaseEntity::GetModelIndex() const {
...
// For debugging shared code
inline bool C_BaseEntity::IsServer() {
	return false;
}

inline bool C_BaseEntity::IsClient() {
	return true;
}

inline const char* C_BaseEntity::GetDLLType() {
	return "client";
}
...
inline int C_BaseEntity::GetSolidFlags() const {
...
// Methods relating to traversing hierarchy
//-----------------------------------------------------------------------------
inline C_BaseEntity* C_BaseEntity::GetMoveParent() const {
	return m_pMoveParent;
}

inline C_BaseEntity* C_BaseEntity::FirstMoveChild() const {
	return m_pMoveChild;
}

inline C_BaseEntity* C_BaseEntity::NextMovePeer() const {
	return m_pMovePeer;
}
...
inline float C_BaseEntity::GetGravity() const {
	return m_flGravity;
}
...
inline float C_BaseEntity::GetElasticity() const {
...
// checks to see if the entity is marked for deletion
//-----------------------------------------------------------------------------
inline bool C_BaseEntity::IsMarkedForDeletion() {
...
//-----------------------------------------------------------------------------
inline Vector C_BaseEntity::EyePosition() const {
	return const_cast<C_BaseEntity*>( this )->EyePosition();
}

inline const QAngle& C_BaseEntity::EyeAngles() const// Direction of eyes in world space
{
	return const_cast<C_BaseEntity*>( this )->EyeAngles();
}

inline const QAngle& C_BaseEntity::LocalEyeAngles() const// Direction of eyes
{
	return const_cast<C_BaseEntity*>( this )->LocalEyeAngles();
}

inline Vector C_BaseEntity::EarPosition() const// position of ears
{
...
inline HTOOLHANDLE C_BaseEntity::GetToolHandle() const {
		...
		return m_ToolHandle;
	#else
		return static_cast<HTOOLHANDLE>( 0 );
	#endif
}
