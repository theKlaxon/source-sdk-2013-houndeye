...
public:
	...
	const char* GetMapNameFilter() { return m_pMapNameFilter; }
	CAchievementMgr* GetAchievementMgr() { return m_pAchievementMgr; }
...
protected:
	...
	const char* m_pInflictorClassNameFilter; // if non-nullptr, inflictor class name to filter with
	const char* m_pInflictorEntityNameFilter;// if non-nullptr, inflictor entity name to filter with
	const char* m_pVictimClassNameFilter;    // if non-nullptr, victim class name to filter with
	const char* m_pAttackerClassNameFilter;  // if non-nullptr, attacker class name to filter with
	const char* m_pMapNameFilter;            // if non-nullptr, map name to filter with
	const char* m_pGameDirFilter;            // if non-nullptr, game dir name to filter with
...
typedef CBaseAchievement* ( *achievementCreateFunc )();
...
#define DECLARE_ACHIEVEMENT_( className, achievementID, achievementName, gameDirFilter, iPointValue, bHidden ) \
	static CBaseAchievement* Create_##className() {                                                            \
