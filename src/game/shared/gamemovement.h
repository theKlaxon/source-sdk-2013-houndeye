...
protected:
	...
	virtual float GetAirSpeedCap() { return 30.f; }
	...
	virtual bool LadderMove();
	virtual bool OnLadder( trace_t& trace );
	virtual float LadderDistance() const { return 2.0f; }///< Returns the distance a player can be from a ladder and still attach to it
	virtual unsigned int LadderMask() const { return MASK_PLAYERSOLID; }
	virtual float ClimbSpeed() const { return MAX_CLIMB_SPEED; }
	virtual float LadderLateralMultiplier() const { return 1.0f; }
	...
	virtual void ReduceTimers();

	virtual void CheckFalling();

	virtual void PlayerRoughLandingEffects( float fvol );

	void PlayerWaterSounds();
