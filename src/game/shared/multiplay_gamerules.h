...
public:
	...
	virtual bool ShouldDrawHeadLabels() {
		if (! mp_show_voice_icons.GetBool() ) {
			return false;
		}
		...
		// Bugbait report
		bool IsLoadingBugBaitReport();

		virtual void ResetMapCycleTimeStamp() { m_nMapCycleTimeStamp = 0; }

		virtual void HandleTimeLimitChange() { return; }
	...
	protected:
		...
		virtual float GetLastMajorEventTime() { return -1.0f; }
...
inline CMultiplayRules* MultiplayRules() {
	return dynamic_cast<CMultiplayRules*>( g_pGameRules );
}
