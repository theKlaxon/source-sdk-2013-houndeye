...
public:
	...
	virtual ~CFlexAnimationTrack();

	void SetEvent( CChoreoEvent* event );
	CChoreoEvent* GetEvent();

	void SetFlexControllerName( const char* name );
	char const* GetFlexControllerName();
...
public:
	...
	const char* GetParameters2();
	void SetParameters3( const char* target );
	const char* GetParameters3();
	...
	void ClearRamp() { m_Ramp.Clear(); };
	void ResortRamp() { m_Ramp.Resort( this ); };
	CCurveData* GetRamp() { return &m_Ramp; };
	...
	bool IsUsingRelativeTag();
	void SetUsingRelativeTag( bool usetag, const char* tagname = 0, const char* wavname = 0 );
	const char* GetRelativeTagName();
	const char* GetRelativeWavName();
