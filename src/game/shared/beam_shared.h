...
public:
	...
	const Vector& GetAbsStartPos() const;
	const Vector& GetAbsEndPos() const;
	...
	int GetType() const;
	int GetBeamFlags() const;
	CBaseEntity* GetStartEntityPtr() const;
	int GetStartEntity() const;
	CBaseEntity* GetEndEntityPtr() const;
	int GetEndEntity() const;
	...
	virtual const char* GetDecalName() { return "BigShot"; }

	#if defined( CLIENT_DLL )
	public: // IClientEntity overrides.
		virtual int DrawModel( int flags );
		virtual bool IsTransparent();
		virtual bool ShouldDraw();
		virtual bool IgnoresZBuffer() const { return true; }
...
inline const Vector& CBeam::WorldSpaceCenter() const {
...
inline CBaseEntity* CBeam::GetStartEntityPtr() const {
	return m_hAttachEntity[ 0 ].Get();
}

inline int CBeam::GetStartEntity() const {
	CBaseEntity* pEntity = m_hAttachEntity[ 0 ].Get();
	return pEntity ? pEntity->entindex() : 0;
}

inline CBaseEntity* CBeam::GetEndEntityPtr() const {
	return m_hAttachEntity[ 1 ].Get();
}

inline int CBeam::GetEndEntity() const {
...
inline int CBeam::GetTexture() {
	return GetModelIndex();
}

inline float CBeam::GetWidth() const {
	return m_fWidth;
}

inline float CBeam::GetEndWidth() const {
	return m_fEndWidth;
}

inline float CBeam::GetFadeLength() const {
	return m_fFadeLength;
}

inline float CBeam::GetNoise() const {
	return m_fAmplitude;
}

inline int CBeam::GetBrightness() const {
	return GetRenderColor().a();
}

inline float CBeam::GetFrame() const {
	return m_fStartFrame;
}

inline float CBeam::GetScrollRate() const {
	return m_fSpeed;
}

inline float CBeam::GetHDRColorScale() const {
