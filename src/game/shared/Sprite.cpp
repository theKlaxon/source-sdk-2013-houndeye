...
#if !defined( CLIENT_DLL )
	int CSprite::UpdateTransmitState() {
	if ( GetMoveParent() ) {
...
void CSprite::TurnOn() {
	...
	// Purpose:
	//-----------------------------------------------------------------------------
	int CSprite::GetRenderBrightness() {
		//See if we're done scaling
...
#else
	bool CSpriteOriented::IsTransparent() {
		return true;
	}
#endif
