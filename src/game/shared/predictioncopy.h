...
public:
	...
	ALWAYS_INLINE void CopyData( difftype_t dt, int size, char* outdata, const char* indata ) { // Copy a binary data block
		if ( !m_bPerformCopy )
			return;
...
public:
	CPredictionDescribeData( void const* src, bool src_packed, FN_FIELD_DESCRIPTION func = nullptr );
