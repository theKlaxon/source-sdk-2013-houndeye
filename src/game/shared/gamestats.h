...
public:
	BasicGameStatsRecord_t() : m_nCount( 0 ),
							   m_nSeconds( 0 ),
							   m_nCommentary( 0 ),
							   m_nHDR( 0 ),
							   m_nCaptions( 0 ),
...
public:
	BasicGameStats_t() : m_nSecondsToCompleteGame( 0 ),
...
public:
	...
	virtual void Event_SaveGame();
	virtual void Event_LoadGame();
	...
	virtual bool StatTrackingEnabledForMod() { return false; }//Override this to turn on the system. Stat tracking is disabled by default and will always be disabled at the user's request
	static bool StatTrackingAllowed();                        //query whether stat tracking is possible and warranted by the user
	virtual bool HaveValidData() { return true; }             // whether we currently have an interesting enough data set to upload.  Called at upload time; if false, data is not uploaded.

	virtual bool ShouldTrackStandardStats() { return true; }//exactly what was tracked for EP1 release
	...
	const char* GetUserPseudoUniqueID();

	virtual bool UserPlayedAllTheMaps() { return false; }//be sure to override this to determine user completion time
		...
		virtual void LoadingEvent_PlayerIDDifferentThanLoadedStats();//Only called if you use the base SaveToFileNOW() and LoadFromFile() functions. Used in case you want to keep/invalidate data that was just loaded.

		virtual bool LoadFromFile();                         //called just before Event_Init()
		...
		virtual bool UploadStatsFileNOW();                   //uploads data to the CSER now, returns success or failure
		...
		virtual bool AutoSave_OnInit() { return false; }
		virtual bool AutoSave_OnShutdown() { return true; }
		virtual bool AutoSave_OnMapChange() { return false; }
		virtual bool AutoSave_OnLevelInit() { return false; }
		virtual bool AutoSave_OnLevelShutdown() { return true; }

		//default upload behavior is to upload on game shutdown
		virtual bool AutoUpload_OnInit() { return false; }
		virtual bool AutoUpload_OnShutdown() { return true; }
		virtual bool AutoUpload_OnMapChange() { return false; }
		virtual bool AutoUpload_OnLevelInit() { return false; }
		virtual bool AutoUpload_OnLevelShutdown() { return false; }
...
public:
	...
	inline bool CBaseGameStats::AppendLump( int nMaxLumpCount, CUtlBuffer& SaveBuffer, unsigned short iLump, unsigned short iLumpCount, size_t nSize, void* pData ) {
		...
		SaveBuffer.Put( pData, static_cast<int>( nTotalSize ) );
	...
	inline void CBaseGameStats::LoadLump( CUtlBuffer& LoadBuffer, unsigned short iLumpCount, size_t nSize, void* pData ) {
		LoadBuffer.Get( pData, static_cast<int>( iLumpCount * nSize ) );
	}
#endif// GAME_DLL
