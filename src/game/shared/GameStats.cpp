...
bool StatsTrackingIsFullyEnabled();
...
CBaseGameStats_Driver::CBaseGameStats_Driver() : BaseClass( "CGameStats" ),
													   m_iLoadedVersion( -1 ),
													   m_bEnabled( false ),
...
CBaseGameStats::CBaseGameStats() : m_bLogging( false ),
								   m_bLoggingToFile( false ) {
}

bool CBaseGameStats::StatTrackingAllowed() {
...
const char* CBaseGameStats::GetStatSaveFileName() {
	AssertMsg( s_szSaveFileName[ 0 ] != '\0', "Don't know what file to save stats to." );
	return s_szSaveFileName;
}

const char* CBaseGameStats::GetStatUploadRegistryKeyName() {
...
const char* CBaseGameStats::GetUserPseudoUniqueID() {
...
void CBaseGameStats::Event_Shutdown() {
	#ifdef GAME_DLL
...
void CBaseGameStats::Event_LevelInit() {
	#ifdef GAME_DLL
...
void CBaseGameStats::Event_SaveGame() {
	StatsLog( "CBaseGameStats::Event_SaveGame [%s]\n", CBGSDriver.m_PrevMapName.String() );
}

void CBaseGameStats::Event_LoadGame() {
	#ifdef GAME_DLL
	...
	bool CBaseGameStats::SaveToFileNOW( bool bForceSyncWrite /* = false */ ) {
		...
		if ( CBGSDriver.m_bShuttingDown || bForceSyncWrite )//write synchronously
		{
			...
			CUtlBuffer statsBuffer( pMem, static_cast<int>( nBufferSize ) );
			statsBuffer.Put( buf.Base(), static_cast<int>( nBufferSize ) );
	...
	bool CBaseGameStats::UploadStatsFileNOW() {
		...
		int curtime = static_cast<int>( Plat_FloatTime() );
		...
		if ( gamestatsuploader ) {
			return gamestatsuploader->UploadGameStats( "", 1, uBlobSize, pvBlobData );
		}
	...
	void CBaseGameStats::LoadingEvent_PlayerIDDifferentThanLoadedStats() {
		StatsLog( "CBaseGameStats::LoadingEvent_PlayerIDDifferentThanLoadedStats\n" );
	}


	bool CBaseGameStats::LoadFromFile() {
		if ( filesystem->FileExists( gamestats->GetStatSaveFileName(), GAMESTATS_PATHID ) ) {
			char fullpath[ 512 ];
...
void OverWriteCharsWeHate( char* pStr ) {
	...
	void CBaseGameStats::SetHL2UnlockedChapterStatistic() {
		...
		if ( filesystem->FileExists( fullpath ) ) {
			...
			if ( FILESYSTEM_INVALID_HANDLE != fh ) {
				...
				int size = static_cast<int>( filesystem->Size( fh ) );
	...
	CPointGamestatsCounter::CPointGamestatsCounter()
			: m_strStatisticName( NULL_STRING ), m_bDisabled( false ) { }
