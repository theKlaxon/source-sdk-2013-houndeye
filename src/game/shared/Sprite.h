...
public:
	...
	void SetGlowProxySize( float flSize ) { m_flGlowProxySize = flSize; }
		...
		void SetAsTemporary() { AddSpawnFlags( SF_SPRITE_TEMPORARY ); }
		bool IsTemporary() { return ( HasSpawnFlags( SF_SPRITE_TEMPORARY ) ); }
	...
	inline float Frames() { return m_flMaxFrame; }
	...
	float GetScale() { return m_flSpriteScale; }
	int GetBrightness() { return m_nBrightness; }
	float GetHDRColorScale() { return m_flHDRColorScale; }
	...
	static CSprite* SpriteCreatePredictable( const char* module, int line, const char* pSpriteName, const Vector& origin, bool animate );
		...
		virtual int GetRenderBrightness();
