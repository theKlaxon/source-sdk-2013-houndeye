...
// CLIENT DLL includes
	...
	#define DECLARE_NETWORKCLASS_NOBASE() \
		...
		#define BEGIN_PREDICTION_DATA( className )                                        \
			...
			datamap_t* className::GetPredDescMap() { return &m_PredMap; }                 \
			BEGIN_PREDICTION_DATA_GUTS( className )

		#define BEGIN_PREDICTION_DATA_NO_BASE( className )                  \
			datamap_t className::m_PredMap = { 0, 0, #className, nullptr }; \
			datamap_t* className::GetPredDescMap() { return &m_PredMap; }   \
			BEGIN_PREDICTION_DATA_GUTS( className )
		...
		#define BEGIN_PREDICTION_DATA_GUTS( className )                                                                                      \
			...
			namespace className##_PredDataDescInit {                                                                                         \
				datamap_t* g_PredMapHolder = PredMapInit( (className*) nullptr ); /* This can/will be used for some clean up duties later */ \
			}                                                                                                                                \
                                                                                                                                             \
			template<>                                                                                                                       \
			datamap_t* PredMapInit<className>( className* ) {                                                                                \
	...
	//  a client side class. Probably could be made into a template at some point.
	#define LINK_ENTITY_TO_CLASS( localName, className )                        \
		static C_BaseEntity* C##className##Factory() {                          \
...
public:
	...
	virtual int GetPredictableCount() = 0;
};
