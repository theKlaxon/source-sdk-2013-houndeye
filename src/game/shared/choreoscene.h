...
public:
	...
	CChoreoActor* AllocActor();
	CChoreoChannel* AllocChannel();
	CChoreoEvent* AllocEvent();
	...
	int GetSceneFPS() const;
	void SetSceneFPS( int fps );
	bool IsUsingFrameSnap() const;
	void SetUsingFrameSnap( bool snap );

	float SnapTime( float t );

	int GetSceneRampCount() { return m_SceneRamp.GetCount(); };
	...
	void ClearSceneRamp() { m_SceneRamp.Clear(); };
	void ResortSceneRamp() { m_SceneRamp.Resort( this ); };

	CCurveData* GetSceneRamp() { return &m_SceneRamp; };
