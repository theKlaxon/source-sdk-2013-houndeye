...
namespace vgui {
	...
	FileCompletionEdit::FileCompletionEdit( Panel* parent ) : TextEntry( parent, nullptr ) {
		m_pDropDown = new FileCompletionMenu( this, nullptr );
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
FileOpenDialog::FileOpenDialog( Panel* parent, const char* title, bool bOpenOnly, KeyValues* pContextKeyValues ) : Frame( parent, "FileOpenDialog" ) {
	m_DialogType = bOpenOnly ? FOD_OPEN : FOD_SAVE;
	Init( title, pContextKeyValues );
}


FileOpenDialog::FileOpenDialog( Panel* parent, const char* title, FileOpenDialogType_t type, KeyValues* pContextKeyValues ) : Frame( parent, "FileOpenDialog" ) {
	m_DialogType = type;
	Init( title, pContextKeyValues );
}
...
void FileOpenDialog::Init( const char* title, KeyValues* pContextKeyValues ) {
	...
	m_pFolderIcon = new ImagePanel( nullptr, "FolderIcon" );
...
#else
	m_pFullPathEdit->AddItem( "/", nullptr );
...
void FileOpenDialog::OnClose() {
	...
	if ( vgui::input()->GetAppModalSurface() == GetVPanel() ) {
		input()->SetAppModalSurface( NULL_PANEL_HANDLE );
	}
...
void FileOpenDialog::PopulateFileNameCompletion() {
	...
	for ( i = 0; i < nCount; i++ ) {
		...
		if ( !_wcsnicmp( wbuf, wszString, bufLen ) ) {
			m_pFileNameEdit->AddItem( wszString, nullptr );
		}
	}
