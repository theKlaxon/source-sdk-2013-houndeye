...
bool BuildGroup::CursorMoved( int x, int y, Panel* panel ) {
	...
	if ( _dragging ) {
		...
		if ( m_hBuildDialog ) {
			...
			ivgui()->PostMessage( m_hBuildDialog->GetVPanel(), keyval, NULL_PANEL_HANDLE );
...
bool BuildGroup::MousePressed( MouseCode code, Panel* panel ) {
	...
	if ( panel == m_hBuildDialog ) {
		...
		ivgui()->PostMessage( m_hBuildDialog->GetVPanel(), new KeyValues( "HideNewControlMenu" ), NULL_PANEL_HANDLE );
		return true;
	}
	...
	if ( code == MOUSE_RIGHT && panel == GetContextPanel() ) {
		...
		ivgui()->PostMessage( m_hBuildDialog->GetVPanel(), new KeyValues( "ShowNewControlMenu" ), NULL_PANEL_HANDLE );
	} else {
		...
		if ( _currentPanel != panel ) {
			...
			if ( m_hBuildDialog ) {
				...
				//ivgui()->PostMessage(m_hBuildDialog->GetVPanel(), new KeyValues("ApplyDataToControls"), nullptr);
		...
		ivgui()->PostMessage( m_hBuildDialog->GetVPanel(), new KeyValues( "StoreUndo" ), NULL_PANEL_HANDLE );
...
bool BuildGroup::MouseReleased( MouseCode code, Panel* panel ) {
	...
	_dragging = false;
	input()->SetMouseCapture( NULL_PANEL_HANDLE );
	return true;
}
...
bool BuildGroup::KeyCodeTyped( KeyCode code, Panel* panel ) {
	...
	switch ( code ) {
		...
		case KEY_DELETE: {
			...
			ivgui()->PostMessage( m_hBuildDialog->GetVPanel(), new KeyValues( "DeletePanel" ), NULL_PANEL_HANDLE );
			break;
		}
	}
	...
	if ( ctrl ) {
		switch ( code ) {
			case KEY_Z: {
				ivgui()->PostMessage( m_hBuildDialog->GetVPanel(), new KeyValues( "Undo" ), NULL_PANEL_HANDLE );
				break;
			}

			case KEY_C: {
				ivgui()->PostMessage( m_hBuildDialog->GetVPanel(), new KeyValues( "Copy" ), NULL_PANEL_HANDLE );
				break;
			}
			case KEY_V: {
				ivgui()->PostMessage( m_hBuildDialog->GetVPanel(), new KeyValues( "Paste" ), NULL_PANEL_HANDLE );
				break;
			}
		}
	}
	...
	if ( dx || dy ) {
		...
		if ( m_hBuildDialog ) {
			...
			ivgui()->PostMessage( m_hBuildDialog->GetVPanel(), new KeyValues( "PanelMoved" ), NULL_PANEL_HANDLE );
		}
	}
...
// Output: Panel *newPanel, nullptr if failed to create new control.
//-----------------------------------------------------------------------------
