...
public:
	...
	explicit operator bool() const { return !BaseClass::operator!(); }
	explicit operator bool() { return !BaseClass::operator!(); }
...
public:
	static int Increment( int* p ) {
		return ThreadInterlockedIncrement( reinterpret_cast<long*>( p ) );
	}
	static int Decrement( int* p ) {
		return ThreadInterlockedDecrement( reinterpret_cast<long*>( p ) );
	}
};
