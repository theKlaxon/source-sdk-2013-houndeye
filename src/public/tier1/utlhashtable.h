...
public:
	...
	ALWAYS_INLINE
	uint32 IdealIndex( uint32 slotmask ) const { return IdealIndex( flags_and_hash, slotmask ) | ( (int32) flags_and_hash >> 31 ); }
	...
	ALWAYS_INLINE
	static uint32 IdealIndex( uint32_if16BitStorage h, uint32 m ) {
		h &= MASK_HASH;
		h *= MASK_HASH + 2;
		return h & m;
	}
	ALWAYS_INLINE
	static uint32 IdealIndex( uint32_if32BitStorage h, uint32 m ) { return h & m; }
