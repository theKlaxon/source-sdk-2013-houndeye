...
enum MemoryPoolGrowType_t {
	...
	UTLMEMORYPOOL_GROW_FAST = 1,// New blob size is numElements * (i+1), so the blocks it allocates
...
public:
	CClassMemoryPool( int numElements, int growMode = GROW_FAST, int nAlignment = 0 ) : CUtlMemoryPool( sizeof( T ), numElements, growMode, MEM_ALLOC_CLASSNAME( T ), nAlignment ) {
