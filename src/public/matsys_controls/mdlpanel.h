...
public:
	...
	virtual void SetMDL( MDLHandle_t handle, void* pProxyData = nullptr );
	virtual void SetMDL( const char* pMDLName, void* pProxyData = nullptr );
	...
	// Attached models.
	void SetMergeMDL( MDLHandle_t handle, void* pProxyData = nullptr, int nSkin = -1 );
	MDLHandle_t SetMergeMDL( const char* pMDLName, void* pProxyData = nullptr, int nSkin = -1 );
	int GetMergeMDLIndex( void* pProxyData );
	int GetMergeMDLIndex( MDLHandle_t handle );
	CMDL* GetMergeMDL( MDLHandle_t handle );
	void ClearMergeMDLs();

	virtual void SetupFlexWeights() { return; }
