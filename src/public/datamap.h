...
#define _FIELD(name,fieldtype,count,flags,mapname,tolerance)		{ fieldtype, #name, { static_cast<int>( offsetof(classNameTypedef,name) ), 0 }, count, flags, mapname, NULL, NULL, NULL, sizeof( ((classNameTypedef *)0)->name ), NULL, 0, tolerance }
...
#define DEFINE_CUSTOM_FIELD(name,datafuncs)	{ FIELD_CUSTOM, #name, { static_cast<int>( offsetof(classNameTypedef,name) ), 0 }, 1, FTYPEDESC_SAVE, NULL, datafuncs, NULL }
