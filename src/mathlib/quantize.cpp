...
static int sortlong( void const* a, void const* b ) {
	...
	return memcmp( ( (struct Sample*) a )->Value, ( (struct Sample*) b )->Value, current_ndims );
}
...
struct QuantizedValue* FindMatch( uint8 const* sample, int ndims,
	...
	PUSHNODE( q );
	for ( ;; ) {
		auto* test = static_cast<struct QuantizedValue*>( RemoveHeapItem( &TheQueue ) );
