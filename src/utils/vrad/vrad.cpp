...
void VRAD_LoadBSP( char const* pFilename ) {
	...
	if ( !g_pFileSystem->FileExists( global_lights ) ) {
		...
		Msg( "Could not find lights.rad in %s.\nTrying VRAD BIN directory instead...\n", global_lights );
			...
			strcpy( global_lights, info.dli_fname );
		#else
			#error "VRAD_LoadBSP: Don't know how to locate shared library file on fs!"
		#endif
	...
	if ( *designer_lights )        // Command-line
		ReadLightFile( designer_lights );
	if ( *level_lights )// Optional & implied
		ReadLightFile( level_lights );
