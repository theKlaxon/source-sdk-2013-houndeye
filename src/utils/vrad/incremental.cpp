...
void CIncremental::AddLightsForActiveLights() {
	...
	for ( directlight_t* dl = activelights; dl != nullptr; dl = dl->next ) {
		...
		pLight->m_flMaxIntensity = std::max( dl->light.intensity[ 0 ], std::max( dl->light.intensity[ 1 ], dl->light.intensity[ 2 ] ) );
	}
}
...
bool CIncremental::LoadIncrementalFile() {
	...
	for ( int iLight = 0; iLight < nLights; iLight++ ) {
		...
		pLight->m_flMaxIntensity = std::max( pLight->m_Light.intensity.x, std::max( pLight->m_Light.intensity.y, pLight->m_Light.intensity.z ) );
