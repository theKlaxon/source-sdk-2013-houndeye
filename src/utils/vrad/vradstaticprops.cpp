...
void Rasterizer::Build() {
	...
	float fMinX = std::min( std::min( mT0.x, mT1.x ), mT2.x );
	float fMinY = std::min( std::min( mT0.y, mT1.y ), mT2.y );
	float fMaxX = std::max( std::max( mT0.x, mT1.x ), mT2.x );
	float fMaxY = std::max( std::max( mT0.y, mT1.y ), mT2.y );
	...
	// Clamp to 0..1
	fMinX = std::max( .0f, fMinX );
	fMinY = std::max( .0f, fMinY );
	fMaxX = std::min( 1.0f, fMaxX );
	fMaxY = std::min( 1.0f, fMaxY );
	...
	iMaxX = std::min( iMaxX, static_cast<int>( mResX - 1 ) );
	iMaxY = std::min( iMaxY, static_cast<int>( mResY - 1 ) );
...
static int GetTexelCount( unsigned int _resX, unsigned int _resY, bool _mipmaps ) {
	...
	while ( _resX > 1 || _resY > 1 ) {
		retVal += _resX * _resY;
		_resX = std::max( 1u, _resX >> 1 );
		_resY = std::max( 1u, _resY >> 1 );
	}
...
static void FilterCoarserMipmaps( unsigned int _resX, unsigned int _resY, CUtlVector<Vector>* _scratchLinear, CUtlVector<RGB888_t>* _outTexelsRGB888 ) {
	...
	int srcResX = _resX;
	int srcResY = _resY;
	int dstResX = std::max( 1, ( srcResX >> 1 ) );
	int dstResY = std::max( 1, ( srcResY >> 1 ) );
	...
	while ( srcResX > 1 || srcResY > 1 ) {
		...
		srcResX = dstResX;
		srcResY = dstResY;
		dstResX = std::max( 1, ( srcResX >> 1 ) );
		dstResY = std::max( 1, ( srcResY >> 1 ) );
...
static void ConvertToDestinationFormat( unsigned int _resX, unsigned int _resY, ImageFormat _destFmt, const CUtlVector<RGB888_t>& _scratchRBG888, CUtlMemory<byte>* _outTexture ) {
	...
	if ( cSrcImageFormat != _destFmt ) {
		while ( srcResX > 1 || srcResY > 1 ) {
			...
			srcResX = std::max( 1, ( srcResX >> 1 ) );
			srcResY = std::max( 1, ( srcResY >> 1 ) );
		}
