...
int CNormalList::FindOrAddNormal( Vector const& vNormal ) {
	...
	for ( int iDim = 0; iDim < 3; iDim++ ) {
		...
		gi[ iDim ] = std::min( gi[ iDim ], static_cast<int>( NUM_SUBDIVS ) );
...
bool CanLeafTraceToSky( int iLeaf ) {
	...
	for ( int j = 0; j < NUMVERTEXNORMALS; j += 4 ) {
		...
		delta.LoadAndSwizzle( g_anorms[ j ], g_anorms[ std::min( j + 1, NUMVERTEXNORMALS - 1 ) ],
							  g_anorms[ std::min( j + 2, NUMVERTEXNORMALS - 1 ) ], g_anorms[ std::min( j + 3, NUMVERTEXNORMALS - 1 ) ] );
...
static const char* ValueForKeyWithDefault( entity_t* ent, const char* key, const char* default_value = nullptr ) {
	epair_t* ep;
...
void BuildFacelights( int iThread, int facenum ) {
	...
	for ( int grp = 0; grp < numGroups; ++grp ) {
		...
		int numSamples = std::min( 4, sampleInfo.m_pFaceLight->numsamples - nSample );
