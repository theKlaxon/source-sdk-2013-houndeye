						...
						bool hasBumpmap, bool neighborHasBumpmap  )
{
	...
	for( s = s_min; s < s_max; s++ )
	{
		...
		{
			float s0 = std::max( coordmins[0] - s, -1.0f );
			float t0 = std::max( coordmins[1] - t, -1.0f );
			float s1 = std::min( coordmaxs[0] - s, 1.0f );
			float t1 = std::min( coordmaxs[1] - t, 1.0f );
...
{
	static FileHandle_t pFpLuxels = nullptr;
	...
	for (k=0 ; k < lightstyles; k++ )
	{
		...
		for (j=0 ; j<fl->numluxels; j++)
		{
			...
			for( bumpSample = 0; bumpSample < bumpSampleCount; bumpSample++ )
			{
				...
				for( i=0; i<3; i++ )
				{
					lb[bumpSample].m_vecLighting[i] = std::max( lb[bumpSample].m_vecLighting[i], minlight );
				}
