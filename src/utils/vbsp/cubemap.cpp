...
static const char* FindSkyboxMaterialName() {
...
static bool LoadSrcVTFFiles( IVTFTexture* pSrcVTFTextures[ 6 ], const char* pSkyboxMaterialBaseName,
	...
	for ( i = 0; i < 6; i++ ) {
		...
		//IMaterialVar *pSkyTextureVar = pSkyboxMaterial->FindVar( bHDR ? "$hdrbasetexture" : "$basetexture", nullptr ); //, bHDR ? false : true );
		IMaterialVar* pSkyTextureVar = pSkyboxMaterial->FindVar( "$basetexture", nullptr );// Since we're setting it to black anyway, just use $basetexture for HDR
		...
		CUtlBuffer buf;
		if ( !g_pFullFileSystem->ReadFile( srcVTFFileName, nullptr, buf ) ) {
			// Try looking for a compressed HDR texture
			if ( bHDR ) {
				/* // FIXME: We need a way to uncompress this format!
				bool bHDRCompressed = true;

				pSkyTextureVar = pSkyboxMaterial->FindVar( "$hdrcompressedTexture", nullptr );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void Cubemap_ResetCubemapSideData() {
...
//-----------------------------------------------------------------------------
void Cubemap_InitCubemapSideData() {
